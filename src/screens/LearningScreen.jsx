...
function CourseComingSoonCard({ courseId, onNavigate }) {
  ...
  return (
    <div style={{ padding: '16px' }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '48px', marginBottom: '10px' }}>{course.emoji}</div>
        <div style={{ fontSize: '11px', fontWeight: 700, color: course.color, letterSpacing: '1.5px', marginBottom: '6px' }}>
          {course.level}
        </div>
        <div style={{ fontSize: '20px', fontWeight: 900, color: '#1e293b', marginBottom: '6px' }}>{course.name}</div>
        <div style={{ fontSize: '14px', color: '#64748b', marginBottom: '16px' }}>{course.subtitle}</div>
        {course.badgeStatus === 'recommended' ? (
          <span style={{
            display: 'inline-block', fontSize: '12px', fontWeight: 800, padding: '5px 14px',
            borderRadius: '20px', background: `${course.color}20`, color: course.color,
            border: `1px solid ${course.color}40`,
          }}>⭐ おすすめ次ステージ</span>
        ) : (
          <span style={{
            display: 'inline-block', fontSize: '12px', fontWeight: 700, padding: '5px 14px',
            borderRadius: '20px', background: '#f1f5f9', color: '#94a3b8',
          }}>🔒 準備中・近日公開</span>
        )}
      </div>
      ...
      <div className="card" style={{ marginBottom: '12px' }}>
        ...
        {[
          ...
          { label: '目的',     value: course.purpose },
          { label: '到達目標', value: course.goal },
          { label: 'レッスン数', value: `${course.lessonCount}レッスン` },
          ...
          <div key={item.label} style={{
            ...
            <div style={{ fontSize: '11px', fontWeight: 700, color: '#94a3b8', width: '68px', flexShrink: 0, paddingTop: '1px' }}>
              {item.label}
            </div>
      ...
      <div className="card" style={{ marginBottom: '12px' }}>
        <div style={{ fontWeight: 800, fontSize: '14px', color: '#1e293b', marginBottom: '12px' }}>
          📚 レッスン一覧（全{course.lessonCount}本）
        </div>
        ...
        {course.lessonCount > 6 && (
          <div style={{ textAlign: 'center', fontSize: '12px', color: '#94a3b8', paddingTop: '10px' }}>
            + さらに{course.lessonCount - 6}本のレッスンを準備中
          </div>
        )}
      </div>
  ...
  return (
    <div style={{ padding: '16px' }}>
      <div style={{
        ...
        <div style={{ fontSize: '18px', fontWeight: 900, color: '#1e293b', marginBottom: '6px' }}>実践ミッション</div>
        <div style={{ fontSize: '13px', color: '#64748b', marginBottom: '14px' }}>実際に動くものを作って学習を定着させる</div>
...
function SpecialLecturesTabContent() {
  ...
  const filtered = activeCat === 'all' ? SPECIAL_LECTURES : SPECIAL_LECTURES.filter((l) => l.category === activeCat);
  ...
  return (
    <div style={{ padding: '16px' }}>
      ...
      <div style={{
        ...
        <div style={{ fontWeight: 900, fontSize: '16px', color: '#1e293b', marginBottom: '4px' }}>特別講座</div>
        <div style={{ fontSize: '12px', color: '#64748b' }}>
          {SPECIAL_LECTURES.length}講座 — 特定ツール・テーマを深掘りする専門コース
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
        {filtered.map((lecture) => (
          <div
            ...
            style={{
              padding: '14px',
              background: `linear-gradient(135deg, ${lecture.color}10 0%, white 100%)`,
              border: `1.5px solid ${lecture.color}25`,
              cursor: 'default',
            }}
          >
            <div style={{ fontSize: '28px', marginBottom: '8px' }}>{lecture.emoji}</div>
            <div style={{ fontWeight: 800, fontSize: '12px', color: '#1e293b', marginBottom: '4px', lineHeight: 1.3 }}>
              {lecture.name}
            </div>
            <div style={{ fontSize: '10px', color: '#94a3b8', marginBottom: '8px', lineHeight: 1.4 }}>
              {lecture.description}
            </div>
            <div style={{
              ...
              <span style={{
                ...
                borderRadius: '6px', background: `${lecture.color}15`, color: lecture.color,
              }}>
                {lecture.category}
              </span>
              <span style={{ fontSize: '10px', color: '#94a3b8', fontWeight: 600 }}>準備中</span>
            </div>
          </div>
        ))}
      </div>
...
export default function LearningScreen({
  ...
  const isNewCourseTab = ['professional', 'business', 'startup', 'graduation'].includes(activeTab);
  ...
  return (
    <div>
      ...
      {/* ===== New Course Tabs (coming soon) ===== */}
      {isNewCourseTab && (
        <CourseComingSoonCard courseId={activeTab} onNavigate={onNavigate} />
      )}
